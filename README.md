...
![前端风格 Skill 提取与验证闭环](assets/workflow.png)

## 为什么需要提取式前端 Vibe Coding

现在用 Agent 做前端已经很方便：可以让模型裸做，也可以加一段 prompt，或者装一些通用 Skills。问题是用多了以后很容易发现，页面常常会越来越像：卡片、渐变、圆角、玻璃态、居中 hero、常规 dashboard，一眼就能看出是模型默认审美。

这个 Skill 换了一个思路：不从空白 prompt 里硬想风格，而是先寻找已经被真实网站验证过的设计案例，再从中蒸馏出前端样式、组件节奏和动效规则。这样既能先看到成品效果，也不需要自己从零搭配审美；提取后再做少量调整，就能得到更有艺术感、更接近真实设计作品的前端风格。

| 方法 | 做法 | 优点 | 常见问题 | 更适合的场景 |
| --- | --- | --- | --- | --- |
| 裸模型生成 | 直接让 Codex、Claude Code 等 Agent 写页面 | 最快，几乎没有准备成本 | 容易落入模型默认审美，页面同质化，艺术感和品牌感不足 | 临时页面、后台小工具、快速验证功能 |
| Prompt 模板 | 在需求前加一段风格 prompt 或 UI 规范 | 比裸写更稳定，能约束颜色、布局和组件 | prompt 很难真正传递复杂视觉细节，复杂动效和材质经常被简化 | 有明确设计方向但不追求强视觉识别度 |
| 通用前端 Skills | 安装一些通用 UI、框架、组件或设计规范 Skills | 能提升工程质量和一致性 | 通常解决“怎么写得规范”，不解决“怎么写得有独特审美” | 组件规范、工程约束、可访问性、响应式质量 |
| 提取式风格 Skill | 从真实网站提取布局、组件、动效、字体层级和视觉节奏，再生成专用 Skill | 先看成品再蒸馏，审美来源更具体，结果更不容易千篇一律 | 需要采集、评分和多轮返工；必须注意版权边界，不能复制原站资产 | 需要艺术感、品牌感、视觉差异化的官网、落地页、作品集、活动页、产品页 |

这个项目主要服务最后一种方法：把“看见一个好网站”变成“得到一个可复用的前端风格 Skill”。
